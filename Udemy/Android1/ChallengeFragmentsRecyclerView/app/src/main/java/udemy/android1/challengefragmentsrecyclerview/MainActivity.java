...
public class MainActivity extends AppCompatActivity implements CarAdapter.ItemClicked {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        buttonFrag = fragmentManager.findFragmentById(R.id.buttonFarg);
        ...
        fragmentManager.beginTransaction().show(buttonFrag).show(listFrag).show(carInfoFrag).hide(ownerInfoFrag).commit();
        ...
        btnOwnerInfo.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                fragmentManager.beginTransaction().show(ownerInfoFrag).hide(carInfoFrag).commit();
            }
        });
        onItemClicked(0);
    }
